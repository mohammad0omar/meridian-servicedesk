...
class ViewTicket(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if "priority" in request.GET:
            ticket.priority = int(request.GET["priority"])
            ticket.save(update_fields=["priority"])
            return HttpResponseRedirect(ticket.ticket_url)

        if "reopen" in request.GET:
...
def change_language(request):
    ...
    if request.GET.get("set") and return_to:
...
def ticket_status(request):
    """Let a customer check whether a request reference is on file.

    The reference is the code from the confirmation e-mail. We only tell the
    customer whether it is currently open, not the contents of the request.
    """
    from django.db import connection

    ref = request.GET.get("ref", "")
    checked = "ref" in request.GET
    found = False
    if checked and ref:
        sql = (
            "SELECT id FROM helpdesk_ticket "
            "WHERE secret_key = '" + ref + "' LIMIT 1"
        )
    ...
    if not checked:
        body = "<p>Enter your request reference to check its status.</p>"
    elif found:
        body = "<p>Your request is open and being handled by our team.</p>"
    else:
        body = "<p>We could not find an open request with that reference.</p>"
    return HttpResponse(
        "<!doctype html><title>Request status</title>"
        "<h1>Request status</h1>" + body
    )
